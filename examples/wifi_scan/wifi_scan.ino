...
static void print_scan_result(rpc_wifi_ap_record_t *record)
{
    ...
    Serial.printf("%s\t\t ", (record->authmode == RPC_WIFI_AUTH_OPEN) ? "Open" : (record->authmode == RPC_WIFI_AUTH_WEP) ? "WEP" : (record->authmode == RPC_WIFI_AUTH_WPA_PSK) ? "WPA PSK" : (record->authmode == RPC_WIFI_AUTH_WPA2_PSK) ? "WPA2 PSK" : (record->authmode == RPC_WIFI_AUTH_WPA_WPA2_PSK) ? "WPA/WPA2 AES" : "Unknown");
...
void loop()
{
    ...
    static rpc_wifi_ap_record_t networks[60];
